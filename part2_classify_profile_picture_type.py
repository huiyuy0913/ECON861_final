...
if not os.path.exists('exam_data/part_2/training_data/test_images'):
  os.mkdir('exam_data/part_2/training_data/test_images/')

class_list = ['buildings', 'dogs', 'faces']
class_list_length  = len(class_list)

for i in class_list:
  if not os.path.exists('exam_data/part_2/training_data/test_images/' + i):
    os.mkdir('exam_data/part_2/training_data/test_images/' + i)


file_list = []
for i in class_list:
  file_list.append([i for i in glob.glob('exam_data/part_2/training_data/image_dataset/' + i + '/*.jpg')])
  ...
  for batch_size in [128, 256]:
    for epochs in [10, 20, 30]:
      for dropout in [0.25, 0.3]:      
        print("\n")
        print(optimizer, batch_size, epochs, dropout)
        print("\n")
        for count in range(split_number):
          ...
          for i in range(class_list_length):
            for j in index_list[i][count][1]:
              ...
              shutil.move(file_list[i][j], 'exam_data/part_2/training_data/test_images/' + class_list[i] + '/' + file_list[i][j].split('/')[-1])


          train_dataset = ImageDataGenerator(rescale = 1/255).flow_from_directory(directory='exam_data/part_2/training_data/image_dataset/', target_size=(30,30), shuffle=True)
          test_dataset = ImageDataGenerator(rescale = 1/255).flow_from_directory(directory='exam_data/part_2/training_data/test_images/', target_size=(30,30), shuffle=True)                 
          ...
          machine.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics='accuracy')

          machine.fit(train_dataset, batch_size=batch_size, epochs=epochs, validation_data=test_dataset) 


          for i in range(class_list_length):
            for j in class_list:
              for file_name in os.listdir('exam_data/part_2/training_data/test_images/'+ j):
                  shutil.move(os.path.join('exam_data/part_2/training_data/test_images/' + j, file_name), 'exam_data/part_2/training_data/image_dataset/' + j)



train_dataset = ImageDataGenerator(rescale = 1/255).flow_from_directory(directory='exam_data/part_2/training_data/image_dataset/',
...
pickle.dump(machine, open('machine_part2_cnn_image.pickle', 'wb'))
