...
machine = pickle.load(open('machine_part3_cnn_image.pickle', 'rb'))


new_data = ImageDataGenerator(rescale = 1/255).flow_from_directory('exam_data/part_3/sample_new_data/',
...
new_data_length = len([i for i in glob.glob("exam_data/part_3/sample_new_data/" + folder_name + "/*.jpg")])
...
sample_new = pandas.read_csv("exam_data/part_3/sample_new_data/" + file_name + ".csv", skipinitialspace=True)
...
with open("machine_part3.pickle", "rb") as f:
...
results['combined_text'] = results['reviewtext'] + ' ' + results['profile']


results_transformed = count_vectorize_transformer.transform(results['combined_text'])
...
results = results.drop('combined_text', axis=1)
print(results)
results.to_csv("exam_data/part_3/sample_new_data/" + file_name + "_data_prediction.csv", index=False, float_format='%.5f')
